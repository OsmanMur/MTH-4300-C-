...
bool bin_search(string arr[], string value, int left_bd, int right_bd){
   int midway = (left_bd + right_bd)/2; // This is an integer! ALSO, when divinding in c++ you round down!!!
   ...
   // RECURSIVE CASE -- 3 possibilities
   if(arr[midway] == value){
      return true;
   } 
   
   else if(arr[midway] < value){
      return bin_search(arr, value, midway + 1, right_bd);
   } 
   else if(arr[midway] > value){ // just else would of been fine too.
