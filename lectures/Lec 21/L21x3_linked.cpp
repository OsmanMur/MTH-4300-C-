...
// LET'S WRITE print() AND push_front()

template <typename S>
void LinkedList<S> :: print() const{
    Node<S>* current = head;
...
template <typename P>
void LinkedList<P> :: push_front(const P&dat){
    head = new Node<P> (dat, head);

}

// pop_front()??
template <typename P>
void LinkedList<P> :: pop_front(){
    if (head != nullptr) {
