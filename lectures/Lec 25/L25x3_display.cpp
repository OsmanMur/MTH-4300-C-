...
// One is easy to fix.  But after we fix it,
// the display() function still doesn't work! 
//

int main() {
    User* my_users[3]; //add a * to make it a pointer
    my_users[0] = Faculty("Evan", "Fink", 12345678, 1000000.0, "Math"); //assigning to a User object limites anything saved in faculty/student/staff & TO FIX this we need to use pointers
    my_users[1] = Student("Johnny","Student", 23456789); 
    my_users[2] = Staff("Jane", "Staffperson", 34567890, 345.00, "Head of HR"); 
    
    for(int i = 0; i < 3; ++i){
        my_users[i]->display();// same as the *: (*my_users[i]).display();
    }
