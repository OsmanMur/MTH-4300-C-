...
Complex Complex::operator+(const Complex &rhs) const { // why &? --> to avoid copying
	double new_real_part = re + rhs.re;				   // why const? --> to avoid unecessary changes
	double new_im_part = im + rhs.im;				   // why const at the end? --> to ensure that the function does not change the object	
